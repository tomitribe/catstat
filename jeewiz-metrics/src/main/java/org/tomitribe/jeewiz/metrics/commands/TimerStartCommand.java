 ...
 *         Created: 5/19/15
 */
public class TimerStartCommand implements Command {
    @Override
    ...
    public void execute(InvocationContext context) {
        ...
        Timer.Context tc = getRegistry(context).timer(name).time();
        context.getContextData().put(Command.METRIC_TIMER+"-"+name, tc);
    }
}
